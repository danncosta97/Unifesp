...
void game_of_life(int **p_grid, int **p_newgrid){
    ...
    for (int gen=0; gen <= GENERATIONS; gen++){
        ...
        # pragma omp parallel for private(i , j,  neighbor_alive, local_alive_cells) shared(alive_cells)
        ...
        //only one thread will going to do this part
        #pragma omp single
        {
        ...
        #pragma omp barrier //sync threads, secure the previous print correctness

        //only one thread will going to do this part
        #pragma omp single
        if(alive_cells!=0) alive_cells=0;

        #pragma omp barrier //sync threads, secure the previous set
    }
