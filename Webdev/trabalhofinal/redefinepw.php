...
<html>
  ...
  <body>
    ...
    <?php
      ...
      if($_POST['redefine']=='Redefinir Senha'){
        if($userpw == ''){
          $error_state=2;
        }
        else{
          $sql = "UPDATE users SET userpw = MD5('$userpw') WHERE idusers = '$idusers'";

          if ($db_connection->query($sql) === TRUE) {
            echo "Record updated successfully";
            $error_state=10;
            //unset($_POST['userpw']);
            header("Location:redefinedpw.php");
            exit();
          } else {
    ...
    <div class="form_box">
      ...
      <form action="redefinepw.php" method="post" autocomplete="off">

        <div class="centertext" style="margin-bottom:10px;"> <?php if($error_state==1) echo '<span style="color:crimson">Ocorreu um erro. Tente novamente mais tarde.</span>';
                                                             else if($error_state==2) echo '<span style="color:crimson">Insira a nova senha!</span>';
                                                             else echo ""; ?>
        </div>

        <div class="centertext" style="width:98%; min-width:120px; display: inline">
          <label for="userpw" style="font-family: Franklin Gothic Medium; width:100%;margin-left:20px;">Senha</label>
        ...
        <div class="centertext">
          <div style="width:48%; min-width:100px; display:inline-block">
            <input type="submit" value="Redefinir Senha" name="redefine" style="width: 100%; margin-top:10px; margin-bottom:10px;">
          </div>
        </div>
