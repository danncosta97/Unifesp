...
int main(int argc, char **argv){
	...
	for(i=2; i<size_dna; i++){
        for(j=0; j<size_dna-i; j++){
            if( dna[j] == dna[j+i]){//printf("if");
                if((table_bases[j+1][j+i] >  table_bases[j][j+i-1]) ||
                   (table_bases[j+1][j+i] == table_bases[j][j+i-1] && table[j+1][j+i] >= table[j][j+i-1])){
                    if((table_bases[j+1][j+i] >  mut[j]+mut[j+i]+table_bases[j+1][j+i-1]) ||
                       (table_bases[j+1][j+i] == (mut[j]+mut[j+i]+table_bases[j+1][j+i-1])
                        && table[j+1][j+i] >= table[j+1][j+i-1])){
                        table[j][j+i] = table[j+1][j+i];
                        table_bases[j][j+i] = table_bases[j+1][j+i];
                    }
                    else{
                        table[j][j+i] = mut[j]+mut[j+i]+table_bases[j+1][j+i-1];
                        table_bases[j][j+i] = 2+table_bases[j+1][j+i-1];
                    }
                }
                else{
                    if((table_bases[j][j+i-1] >  mut[j]+mut[j+i]+table_bases[j+1][j+i-1]) ||
                       (table_bases[j][j+i-1] == (mut[j]+mut[j+i]+table_bases[j+1][j+i-1])
                        && table[j][j+i-1] >= table[j+1][j+i-1])){
	...
	if(most_bases<=table_bases[j][j+i] && longest_palindrome<table[j][j+i]){
                longest_palindrome=table[j][j+i];
            }
    k=0; l=0;
    for(k=0; k<size_dna; k++){
        for(l=k; l<size_dna; l++){
            if(most_bases<table_bases[k][l]){
                ...
                most_bases=table[k][l];
            }
            else if(most_bases==table_bases[k][l]){
